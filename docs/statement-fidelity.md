> **⚠ 2026-09-18 更新:条件性已经消除。**
> 本文写于交付时,当时主定理带 `hcurve`(论文 Lemma 2.2)这条显式假设。该假设现已被
> `Erdos960.curveModelAssumption` 证明,顶层定理 `Erdos960.erdos960_unconditional` 除论文
> 自身的 `r ≥ 3`、`k ≥ 4`、`n ≥ 72` 外没有任何前提。下文凡把交付描述为「条件形式化」的
> 段落按当时状态保留。构型改用奇异三次曲线而非论文的椭圆曲线,详见根目录 README。
