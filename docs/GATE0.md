> **⚠ 2026-09-18 更新:本报告的判定已被超越。**
> 这里的结论「Lemma 2.2 绕开、成为假设」只针对论文那条路线,即在光滑曲线 `y² = x³ − x + 1`
> 上走 `E(ℝ) ≅ ℝ/ℤ`。该判定本身仍然成立:Mathlib 至今对
> `WeierstrassCurve.Affine.Point` 没有任何拓扑结构。
>
> 但 `CurveModel` 要的比论文用的弱,它只要求*某个*群到平面的嵌入使共线等于群和为零。
> `Erdos960/Nodal.lean` 换用**奇异**三次曲线 `Z(X² + Y²) = X³`,其群律是纯三角,
> 于是 Lemma 2.2 被**证明**了(`Erdos960.curveModelAssumption`),主定理
> `erdos960_unconditional` 不再带任何曲线侧假设。本文件作为可行性判定的历史记录保留。
